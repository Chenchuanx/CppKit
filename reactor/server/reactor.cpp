...
// 广播
void Reactor::Broadcast(int fd, const std::string& recv_data) {
    const size_t CHUNK_SIZE = 1024;  
    std::vector<std::string> send_datas;
    
    // 预分配空间
    send_datas.reserve((recv_data.size() + CHUNK_SIZE - 1) / CHUNK_SIZE);
    
    // 分割字符串
    for (size_t i = 0; i < recv_data.size(); i += CHUNK_SIZE) {
        size_t chunk_size = std::min(CHUNK_SIZE, recv_data.size() - i);
        send_datas.emplace_back(recv_data.substr(i, chunk_size));
    }
    // std::cout << "Broadcasting data to all clients... " << send_datas.size() << std::endl;
    ...
    for (auto& active_fd : active_fds_) {
        ...
        for (const std::string& data : send_datas) {
            ssize_t bytes_written = write(active_fd, data.c_str(), data.size());
            if (bytes_written == -1) {
                std::cerr << "Write error: " << strerror(errno) << std::endl;   // 可能由于缓存区
            }
        }
    }
}
